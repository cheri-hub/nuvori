...
export function MuruScene({ lineProgress, mood, stage = 1 }: MuruSceneProps) {
  const progress = Math.min(1, Math.max(0, lineProgress));
  return (
    <div className={`muru-scene mood-${mood} muru-stage-${stage}`} data-muru-stage={stage} style={{ '--line-progress': progress } as React.CSSProperties}>
      <img className={`muru-image muru-image--${mood}`} src="/assets/muru-idle.svg" alt="Muru, seu companheiro de jornada" />
